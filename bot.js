...
client.on('ready', async () => {
 ...
 client.user.setActivity(`RAVENCLAW ÖĞRENCİLERİNİ`, { type:'WATCHING' }) //PLAYING= OYNUYOR, WHATCHING= İZLİYOR, LISTENING= DİNLİYOR
...
client.on('guildMemberAdd', async (member) => {
    var kanal = member.guild.channels.get('') //Mesajın atılmasını istediğiniz kanalın id sini girin.
    kanal.send(`MESAJ`) ;
})
...
fs.readdir('./Puan/', (err, files) => {
    if (err) console.error(err);
    log(`${files.length} Puan komutu Yükleniyor...`);
    files.forEach(f => {

        let props = require(`./Puan/${f}`);
        log(`Puan Komutu Yüklendi: ${props.help.name}.`);
...
//SA AS
let pp = ["Olmayan ne varsa hepsine selam olsun. Merhaba!", "Selam vermek sünnet, selam almak farzdır. Sünneti sizden farzı bizden olsun.", "Bavv çiki bav baa-v ay pardon geldiğini görmedim"]
let prkr = pp[Math.floor(Math.random() * pp.length)];

client.on("message", async msg => {
    if(msg.content.toLowerCase() === "merhaba") {
        return msg.inlineReply(prkr);
    }
});
client.on("message", async msg => {
...
client.on("message", async msg => {
    if(msg.content.toLowerCase() === "merhabaa") {
        return msg.inlineReply(prkr);
    }
});
...
client.on("message", async message =>  {
    ...
    let array = ["Buyurun", "Efendim?", "Ne oldu?", "Efendim Canım"]
    let çıktı = array[Math.floor(Math.random() * array.length)];
   
   return message.inlineReply(çıktı);
    }  
